...
fn main() {
    ...
    let s1  = search_artists(artists.clone(), vec!["Pop"], vec!["England"], true, 1950, 2022);
    for s in s1 {
        println!("{:?}", s);
    }
    
    // Search for artists from England active between 1950 and 2022
    let s1 = search_artists(artists.clone(), vec![], vec!["England"], true, 1950, 2022);
    for s in s1 {
        println!("{:?}", s);
    }
    ...
    let s1 = search_artists(artists.clone(), vec!["Heavy Metal"], vec![], false, 2019, 2022);
    for s in s1 {
        println!("{:?}", s);
    }
    
    // Search for artists from the U.S. active between 1950 and 1959
    let s1 = search_artists(artists.clone(), vec![], vec!["U.S."], true, 1950, 1959);
    for s in s1 {
        println!("{:?}", s);
    }
